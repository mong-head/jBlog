				...
				<c:import url="/WEB-INF/views/includes/blog/menu.jsp" />
		      	<table class="admin-cat">
		      		...
		      		<c:forEach var="vo" items="${categoryList }" varStatus="status">
			      		<tr>
							<td>${listLength - status.index }</td>
							<td>${vo.name }</td>
							<td>${vo.count }</td>
							<td>${vo.description }</td>
							<td><a href="${pageContext.request.contextPath }/${authUser.id }/admin/category/delete?no=${vo.no}"><img src="${pageContext.request.contextPath}/assets/images/delete.jpg" /></a></td>
						</tr>  
		      		</c:forEach>
				</table>
